...
export class CostoComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        if (this.currentSearch) {
            this.costoService.search({
                query: this.currentSearch,
                page: this.page,
                size: this.itemsPerPage,
                sort: this.sort()
            }).subscribe(
                (res: HttpResponse<Costo[]>) => this.onSuccess(res.body, res.headers),
    ...
    trackId(index: number, item: Costo) {
        return item.id;
    }
    registerChangeInCostos() {
        this.eventSubscriber = this.eventManager.subscribe('costoListModification', (response) => this.reset());
    }
